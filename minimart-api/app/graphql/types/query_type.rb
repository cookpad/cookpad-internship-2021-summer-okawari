module Types
  class QueryType < Types::BaseObject
    ...
    # 該当する文字をnameかdescriptionに含む商品を返すクエリ searchProducts(word: String!) の定義
    field :search_products, [ProductType], '指定された文字をnameに含む商品を返す', null: true do
      argument :query, String, required: true
    end
    def search_products(query:)
      Product.where("(name like '%#{query}%') OR (description like '%#{query}%')")
    end
    ...
    # 指定されたIDの商品を返すクエリ Product(id: ID!) の定義
    field :category, CategoryType, '指定されたIDのカテゴリに含まれる商品を返す', null: true do
    argument :id, ID, required: true
    end
    def category(id:)
